...
- **Fichiers Vidéo Corrompus (Timeout) :** Remplacement de la terminaison forcée de `ffmpeg` par un signal d'arrêt propre (écriture de `q` sur `stdin`) et l'utilisation de conteneurs fragmentés (`frag_keyframe`). Cela empêche la corruption des fichiers `.mp4` lors des coupures réseau.
- **Robustesse et Réessai (Auto-Retry) :** Ajout d'une boucle de réessai automatique (jusqu'à 2 tentatives espacées d'une seconde) en cas de capture vide ou de timeout réseau. Le processus de surveillance globale a été blindé contre toute levée d'exception pour ne plus jamais figer la détection des autres caméras.
- **Diagnostic Intégré :** Journalisation de la sortie d'erreur de `ffmpeg` dans `ffmpeg_CamX.log`. Si le fichier vidéo s'avère illisible par l'IA, le système extrait automatiquement la fin du log pour l'afficher dans les erreurs globales du backend.
